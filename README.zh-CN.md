...
```
https://<host>/live/<房间名>                   # broadcaster
https://<host>/live/<房间名>/viewer.html       # viewer
```

**多房间支持：**不同房间名（如 `/live/saki`、`/live/ick`）完全隔离，同一台服务器可同时服务多对 broadcaster-viewer。
...
1. **URL 查询参数（推荐）：**
   ```
   https://<host>/live/<房间名>/viewer.html?token=xxxx
   ```
   系统自动鉴权，无需额外操作。
...
TURN 凭据由信令服务器在房间加入后通过 WebSocket 动态下发，无需在 URL 中指定。
...
**TURN 凭据由信令服务器在房间加入后通过 WebSocket 动态下发**，不会暴露在静态文件或 URL 中。启动服务器前设置环境变量：

| 变量 | 示例 | 说明 |
|------|------|------|
| `TURN_HOST` | `47.113.225.81` | TURN 服务器 IP 或域名 |
| `TURN_SECRET` | `your-secret-here` | HMAC 凭据生成的共享密钥（需与 coturn `static-auth-secret` 一致） |
| `TURN_CREDENTIAL_TTL` | `300` | 凭据有效期（秒，默认 300 = 5 分钟） |

当 `TURN_HOST` 和 `TURN_SECRET` 均设置后，信令服务器为每个房间生成临时 HMAC-SHA1 凭据，通过 `joined` WebSocket 消息下发。每个凭据的有效期为 `TURN_CREDENTIAL_TTL` 秒。
...
```
https://<host>/live/<房间名>                   # broadcaster
https://<host>/live/<房间名>/viewer.html?token=xxxx  # viewer
```
...
生成测试凭据（将 `<密钥>` 替换为你的 `static-auth-secret`）：
...
turnutils_uclient -t -u "$USERNAME" -w "$PASSWORD" -p 3478 <公网IP>
```
...
1. **打开浏览器 DevTools → Network → WS 标签。**检查 `joined` 消息 — 确认 `iceServers` 包含 TURN URL、用户名、密码
...
4. **无 `srflx` candidate？**STUN 不可达 — 多个 STUN 服务器通过信令下发作为回退
